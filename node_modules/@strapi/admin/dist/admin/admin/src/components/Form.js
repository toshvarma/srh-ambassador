 ...
 */ const Form = /*#__PURE__*/ React__namespace.forwardRef(({ disabled = false, method, onSubmit, initialErrors, ...props }, ref)=>{
    ...
    const valuesRef = React__namespace.useRef(state.values);
    /**
    ...
    const getValues = React__namespace.useCallback(()=>valuesRef.current, []);
    ...
    const modified = React__namespace.useMemo(()=>!isEqual(initialValues.current, state.values), // eslint-disable-next-line react-hooks/exhaustive-deps
    [
        state.values,
    ...
    const resetForm = React__namespace.useCallback((newInitialValues)=>{
        if (newInitialValues) {
...
const reducer = (state, action)=>immer.produce(state, (draft)=>{
         ...
         */ const currentField = objects.getIn(state.values, action.payload.field, []);
                    ...
                    do {
                        [key] = fractionalIndexing.generateNKeysBetween(lowerBound, null, 1);
