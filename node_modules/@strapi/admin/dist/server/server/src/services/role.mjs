...
import { omit, pick, isArray, set, differenceWith, differenceBy, prop, isEqual } from 'lodash/fp';
import { hooks as hooks$1, errors, arrays, dates } from '@strapi/utils';
 ...
 */ const assignPermissions = async (roleId, permissions = [])=>{
    ...
    const internalActions = getService('permission').actionProvider.values().filter((action)=>action.section === 'internal').map((action)=>action.actionId);
