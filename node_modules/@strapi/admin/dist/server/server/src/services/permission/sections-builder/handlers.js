...
const { isVisibleAttribute } = utils.contentTypes;
/**
 ...
 */ const subjectsHandlerFor = (kind)=>({ action, section: contentTypesSection })=>{
        ...
        .filter(utils$1.isOfKind(kind))// Transform the content-types into section's subjects
 ...
 */ const fieldsProperty = ({ action, section })=>{
    ...
    section.subjects.filter((subject)=>subjects?.includes(subject.uid)).forEach((subject)=>{
        const { uid } = subject;
        const contentType = utils$1.resolveContentType(uid);
        if (utils$1.hasProperty('fields', subject)) {
            return;
        }
