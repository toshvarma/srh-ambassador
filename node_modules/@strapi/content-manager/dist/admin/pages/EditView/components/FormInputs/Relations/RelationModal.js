...
const RootRelationRenderer = (props)=>{
    ...
    const documentLayoutResponse = useDocumentLayout.useDocumentLayout(rootDocument.model);
    const relationOpenMode = documentLayoutResponse.edit?.settings?.relationOpenMode ?? 'modal';
...
const RelationModalTrigger = ({ children, relation })=>{
    ...
    if (relationOpenMode === 'newTab') {
        return /*#__PURE__*/ jsxRuntime.jsx(StyledRelationLink, {
            to: fullPageUrl,
            target: "_blank",
...
`;
const StyledRelationLink = styledComponents.styled(reactRouterDom.Link)`
  max-width: 100%;
  font-size: ${({ theme })=>theme.fontSizes[2]};
  overflow: hidden;
  white-space: nowrap;
...
exports.generateCreateUrl = generateCreateUrl;
exports.getCollectionType = getCollectionType;
exports.getFullPageUrl = getFullPageUrl;
