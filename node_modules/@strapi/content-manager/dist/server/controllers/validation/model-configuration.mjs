...
import { isListable, hasEditableAttribute } from '../../services/utils/configuration/attributes.mjs';
import { isValidDefaultSort } from '../../services/utils/configuration/settings.mjs';

/**
 * Creates the validation schema for content-type configurations
 */ var createModelConfigurationSchema = ((schema, opts = {})=>yup.object().shape({
...
const createSettingsSchema = (schema)=>{
    ...
    return yup.object().shape({
        ...
        // should be reset when the type changes
        defaultSortBy: yup.string().test('is-valid-sort-attribute', '${path} is not a valid sort attribute', async (value)=>isValidDefaultSort(schema, value)).default('id'),
        defaultSortOrder: yup.string().oneOf([
            'ASC',
            'DESC'
        ]).default('ASC'),
        relationOpenMode: yup.string().oneOf([
            'modal',
            'page',
            'newTab'
        ]).default('modal')
...
const createMetadasSchema = (schema)=>{
    return yup.object().shape(Object.keys(schema.attributes).reduce((acc, key)=>{
        acc[key] = yup.object().shape({
            edit: yup.object().shape({
                ...
                mainField: yup.lazy((value)=>{
                    ...
                    const targetSchema = getService('content-types').findContentType(schema.attributes[key].targetModel);
                    if (!targetSchema) {
                        return yup.string();
                    }
...
const createLayoutsSchema = (schema, opts = {})=>{
    ...
    return yup.object().shape({
        edit: yup.array().of(yup.array().of(yup.object().shape({
...
export { createModelConfigurationSchema as default };
//# sourceMappingURL=model-configuration.mjs.map
