import { uniqBy, concat, isEmpty, uniq, flow, prop } from 'lodash/fp';
import { contentTypes, relations as relations$1, isOperatorOfType, errors } from '@strapi/utils';
import { getService } from '../utils/index.mjs';
import { validateFindExisting, validateFindAvailable } from './validation/relations.mjs';
import { isListable } from '../services/utils/configuration/attributes.mjs';
...
var relations = {
    async extractAndValidateRequestInfo (ctx, id) {
        ...
        const modelConfig = isComponent ? await getService('components').findConfiguration(sourceSchema) : await getService('content-types').findConfiguration(sourceSchema);
   ...
   */ async findAvailable (ctx) {
    ...
    async findExisting (ctx) {
        ...
        const permissionQuery = await getService('permission-checker').create({
            userAbility,
            model: targetUid
        }).sanitizedQuery.read({
            fields: fieldsToSelect
