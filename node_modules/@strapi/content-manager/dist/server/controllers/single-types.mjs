...
const buildPopulateFromQuery = async (query, model)=>{
    return getService('populate-builder')(model).populateFromQuery(query).populateDeep(Infinity).countRelations().withPopulateOverride(getPopulateForLocalizations(model)).build();
};
...
var singleTypes = {
    ...
    async publish (ctx) {
        ...
        const publishedDocument = await strapi.db.transaction(async ()=>{
            ...
            let document = await findDocument(sanitizedQuery, model, {
                locale,
                status: 'draft'
            });
            ...
            if (shouldUpdate || shouldCreate) {
                document = await createOrUpdateDocument(ctx, {
                    populate
                });
            } else if (!document) {
