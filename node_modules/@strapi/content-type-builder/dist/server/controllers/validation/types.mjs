...
import { VALID_UID_TARGETS } from '../../services/constants.mjs';
import { validators, isValidName, isValidEnum, areEnumValuesUnique, isValidDefaultJSON, isValidRegExpPattern, UID_REGEX } from './common.mjs';
        ...
        case 'uid':
            {
                return {
                    ...
                    }).test('isValidDefaultRegexUID', `\${path} must match the custom regex or the default one "${UID_REGEX}"`, function(value) {
                        const { regex } = this.parent;
                        if (regex) {
                            return !_.isNil(value) && (value === '' || new RegExp(regex).test(value));
                        }
                        return value === '' || UID_REGEX.test(value);
                    }),
