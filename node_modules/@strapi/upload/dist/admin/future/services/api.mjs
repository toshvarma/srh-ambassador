...
import { getFilenameFromUrl } from '../utils/files.mjs';
 ...
 */ const fetchUrlUploadStream = async ({ token, urls, folderId, signal })=>{
    const backendURL = window.strapi.backendURL;
    const headers = {
 ...
 */ const performStreamUpload = async ({ token, formData, abortController, uploadId, dispatch, indexMapper = (i)=>i, onUploadFailed = (message)=>dispatch(setUploadFailed({
        message
    })) })=>{
    try {
...
const { useUploadFilesStreamMutation, useRetryCancelledFilesStreamMutation, useUploadFromUrlsMutation } = uploadApi;

export { abortUpload, uploadApi, useRetryCancelledFilesStreamMutation, useUploadFilesStreamMutation, useUploadFromUrlsMutation };
